...
void gl_convertRGB_to_5R6G5B(uint16_t *pixmap, uint8_t *rgb, int xsize,
                             int ysize) {
  int i, n;
  uint8_t *p;
...
void gl_convertRGB_to_8A8R8G8B(uint32_t *pixmap, uint8_t *rgb, int xsize,
                               int ysize) {
  int i, n;
  uint8_t *p;
...
void gl_resizeImage(uint8_t *dest, int xsize_dest, int ysize_dest, uint8_t *src,
  ...
  for (y = 0; y < ysize_dest; y++) {
    ...
    for (x = 0; x < xsize_dest; x++) {
      ...
      if ((xf + yf) <= INTERP_NORM) {
        for (j = 0; j < 3; j++) {
          pix[j] =
              interpolate(pix_src[(yi * xsize_src + xi) * 3 + j],
        ...
        for (j = 0; j < 3; j++) {
          pix[j] =
              interpolate(pix_src[((yi + 1) * xsize_src + xi + 1) * 3 + j],
